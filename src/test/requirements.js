export default function requirements(mode) {
  ...
  const requirement = {
    ...
    Response of PokeAPI will be in the following format:{ results: [ { name: "bulbasaur" }, { name: "ivysaur" }, { name: "venusaur" }...] }`,
  };

  const userstory = {
    easy: `Message 'Want to buy a new car?' is displayed on the page.
    Text 'Call +11 22 33 44 now!' is not visible at first.
    User clicks toggle message.
    Text 'Call +11 22 33 44 now!' will appear below the message.
    Text 'Call +11 22 33 44 now!' will disappear when user clicks the message again.`,
    medium: `User input number to two text fields.  // e.g. 1 and 5
    User click "add" button.
    Adding result should be displayed below the button. // e.g. 6`,
    hard: `User can see 10 Pokemon name.
    They should be displayed as follows on the page: bulbasaur ivysaur venusaur ...`,
  };
